...
@section('content_header')
    <h1>Asignar establecimientos y módulos a usuarios</h1>
@stop
@section('content')
    <div class="card">
        <div class="card-body">
        ...
        <div class="table-responsive" style="max-height: 600px;">
            <table class="table table-bordered table-striped" id="tabla-asignacion-admin" style="width:100%;">
...
<div class="modal fade" id="modalAsignacionAdmin" data-backdrop="static" tabindex="-1" role="dialog">
        ...
        <form id="form-asignacion-admin">
            ...
            <div class="modal-content">
                ...
                <div class="modal-header">
                        <h5 class="modal-title" id="modalAsignarEstablecimientoAdminLabel">Asignar establecimiento a Usuario</h5>
                ...
                <div class="modal-body row">
                    <div class="form-group col-md-12 mb-2">
                        ...
                        @foreach(
                            \App\Models\User::where('status', true)
                    ...
                    <div class="form-group col-md-12 mb-2">
                        ...
                        @foreach(
                            \App\Models\Establecimiento::whereIn('id', function ($query) {
                                $query->select('establecimiento_id')
                                    ->from('establecimiento_usuario')
                                    ->where('user_id', auth()->id());
                            })->orderBy('nombre_comercial')->get() as $establecimiento)
...
$('#btn-toggle-filtros').on('click', function () {
    ...
    //para asignar una establecimiento al usuario nuevo
        ...
        $('#modalAsignarEstablecimientoAdminLabel').html('<i class="fas fa-clipboard-check text-success mr-2"></i> Nuevo establecimiento a usuario');
    ...
    $('#form-asignacion-admin').submit(function (e) {
        e.preventDefault();
        $.post('{{ route("asignacion_establecimiento_usuario_admin.asignar") }}', $(this).serialize(), function (response) {
            if (response.success) {
...
$(document).on('click', '.modulos-asignacion-admin', function () {
    ...
    $.post('{{ route("asignacion_establecimiento_usuario_admin.permisos") }}', {
        _token: '{{ csrf_token() }}',
        user_id: userId,
        ...
        $('#tabla-permisos-container').html('<div class="text-danger">' + mensaje + '</div>');
    });
});
...
$(document).on('click', '#btn-aplicar-perfil', function () {
    const perfil = new URLSearchParams(localStorage.getItem('ultimo_perfil_permisos-admin'));
...
$('#form-modulos-admin').submit(function (e) {
    ...
    $.post('/asignacion_establecimiento_usuario_admin/permisos/guardar', datos, function () {
